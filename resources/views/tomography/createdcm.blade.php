...
<div class="flex justify-end"><a href="{{ route('tomography.index') }}" class="botton1">Tomografias</a></div>
    <form method="POST" action="{{ route('tomography.storedcm') }}" enctype="multipart/form-data">
        ...
        <div class="flex items-center mb-4"><label class="txt1">Subir archivo:</label>
        <input type="file" name="tomography_folder[]" webkitdirectory directory multiple class="border-gray-300 dark:border-gray-600 rounded-lg p-2 w-full text-black dark:text-black focus:outline-none focus:ring-2 focus:ring-cyan-500 mr-10" />
        </div>
